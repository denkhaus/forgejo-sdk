...
func (c *Client) ListPublicKeys(user string, opt ListPublicKeysOptions) ([]*models.PublicKey, *Response, error) {
	if err := escapeValidatePathSegments(&user); err != nil {
		return nil, nil, err
	}
	opt.setDefaults()
	keys := make([]*models.PublicKey, 0, opt.PageSize)
	resp, err := c.getParsedResponse("GET", fmt.Sprintf("/users/%s/keys?%s", user, opt.getURLQuery().Encode()), nil, nil, &keys)
	return keys, resp, err
}

// ListMyPublicKeys list all the public keys of current user
func (c *Client) ListMyPublicKeys(opt ListPublicKeysOptions) ([]*models.PublicKey, *Response, error) {
	opt.setDefaults()
	keys := make([]*models.PublicKey, 0, opt.PageSize)
...
func (c *Client) GetPublicKey(keyID int64) (*models.PublicKey, *Response, error) {
...
func (c *Client) CreatePublicKey(opt CreateKeyOption) (*models.PublicKey, *Response, error) {
